...
public final class ExperimentUtils {
    ...
    public static ExperimentStats runExperiment(Coordinator coordinator, Tuple<Integer, String>[] txRatio,
        ...
        if (CONSUMER_REGISTERED) {
            BATCH_TO_FINISHED_TS_MAP.keySet().stream().max(Long::compareTo).ifPresent(
                    highestKey -> lastExperimentLastTID =
                            (int) BATCH_TO_FINISHED_TS_MAP.get(highestKey).lastTid);
            BATCH_TO_FINISHED_TS_MAP.clear();
        } else {
            coordinator.registerBatchCommitConsumer((batchId, tid) ->
                    BATCH_TO_FINISHED_TS_MAP.put(batchId,
                            new BatchStats(batchId, tid, System.currentTimeMillis())));
            CONSUMER_REGISTERED = true;
        }

        int newRuntime = runTime + warmUp;
        WorkloadUtils.WorkloadStats workloadStats =
                WorkloadUtils.submitWorkload(txRatio, input, func, newRuntime);
        ...
        return new ExperimentStats(workloadStats.initTs(), runTime, usefulRuntime,
                numCompletedWithWarmUp, numCompleted, txPerSec, txPerSecUseful,
                average, percentile_50, percentile_75, percentile_90, percentile_99);
    }

    public record ExperimentStats(long initTs, int runTime, long usefulRuntime,
                                  int numCompletedWithWarmUp, int numCompleted,
                                  double txPerSec, double txPerSecUseful,
                                  double average, double percentile_50,
    ...
    public static void writeResultsToFile(int numWare, ExperimentStats expStats, int runTime, int warmUp,
                                          ...
                                          String logging, String checkpointing) {
        LocalDateTime time = LocalDateTime.ofInstant(
                Instant.ofEpochMilli(expStats.initTs), ZoneId.systemDefault());
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) {
            writer.write("======= TPC-C in vMODB ======="); writer.newLine();
            writer.write("Start: " + formattedDate); writer.newLine();
            writer.write("Duration (ms): " + runTime); writer.newLine();
            writer.write("Warm up (ms): " + warmUp); writer.newLine();
            writer.write("Useful work (ms): " + expStats.usefulRuntime); writer.newLine();
            writer.write("Batch window (ms): " + batchWindow); writer.newLine();
            writer.write("Max transactions per batch: " + maxTransactionsPerBatch); writer.newLine();
            writer.write("Number of transaction workers: " + numTransactionWorkers); writer.newLine();
            writer.write("Number of warehouses: " + numWare); writer.newLine();
            writer.write("Transaction ratio: "); writer.newLine();
            for (Tuple<Integer, String> tx : txRatio) {
                writer.write("  " + tx.t2 + "=" + tx.t1); writer.newLine();
            }
            ...
            writer.write("Latency at 75th percentile: " + expStats.percentile_75); writer.newLine();
            writer.write("Latency at 90th percentile: " + expStats.percentile_90); writer.newLine();
            writer.write("Latency at 99th percentile: " + expStats.percentile_99); writer.newLine();
            writer.write("Number of completed transactions (during warm up): "
                    + (expStats.numCompletedWithWarmUp - expStats.numCompleted)); writer.newLine();
            writer.write("Number of completed transactions (after warm up): "
                    + expStats.numCompleted); writer.newLine();
            writer.write("Number of completed transactions (total): "
                    + expStats.numCompletedWithWarmUp); writer.newLine();
            writer.write("Throughput (tx/sec): " + expStats.txPerSec); writer.newLine();
            writer.write("Useful Throughput (tx/sec): " + expStats.txPerSecUseful); writer.newLine();
        } catch (IOException e) {
