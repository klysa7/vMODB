 ...
 *   - Added freshness table configuration (max_records + chaining)
 *   - Added IFreshnessRepository to VmsApplication.build() callback
 */
public final class Main {
    ...
    public static VmsApplication build() throws Exception {
        ...
        // ── HATtrick: freshness table — one row per T-client ──────────────
        // num_t_clients defaults to 2 if not set in config
        int numTClients = Integer.parseInt(prop.getProperty("num_t_clients", "2"));
        prop.setProperty("max_records.freshness",     String.valueOf(numTClients));
        prop.setProperty("table.freshness.chaining",  "false");
        // ──────────────────────────────────────────────────────────────────
        ...
        return VmsApplication.build(options, (x, y) -> new OrderHttpHandler(x,
                ...
                (IFreshnessRepository) y.apply("freshness")   // ← HATtrick addition
        ));
    }
}