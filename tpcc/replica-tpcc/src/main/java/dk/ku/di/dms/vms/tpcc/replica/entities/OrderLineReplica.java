...
/**
 * Replica copy of the order_line table. Same 10 columns as the live OrderLine
 * (including ol_delivery_d) so vMODB's metadata loader maps it identically;
 * composite PK (ol_o_id, ol_d_id, ol_w_id, ol_number). the
 * replica holds only order_line.
 */
@Entity
...
public final class OrderLineReplica implements IEntity<OrderLineReplica.OrderLineReplicaId> {
    ...
    public OrderLineReplica(int ol_o_id, int ol_d_id, int ol_w_id, int ol_number,
                            int ol_i_id, int ol_supply_w_id,
