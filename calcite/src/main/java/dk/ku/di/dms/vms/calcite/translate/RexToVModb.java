package dk.ku.di.dms.vms.calcite.translate;
...
public final class RexToVModb {
    ...
    /** Placeholder for ?0, ?1, ... to be bound at execution time */
    ...
    public static FilterContext toFilterContext(RexNode condition) {
        ...
        ctx.predicates = new ArrayList<>(); // not used yet
        ...
        for (RexNode c : conjuncts) {
            if (!(c instanceof RexCall call)) {
                throw new UnsupportedOperationException("Unsupported predicate (not a call): " + c);
            }
            if (call.getKind() != SqlKind.EQUALS) {
                throw new UnsupportedOperationException("Only '=' supported for now: " + call);
            }
            ...
            if (left instanceof RexInputRef lcol) {
                ...
                throw new UnsupportedOperationException("Only (col = literal/param) supported: " + call);
            }
    ...
    private static Object extractLiteralOrParam(RexNode node) {
        ...
        throw new UnsupportedOperationException("Only literal or ?param supported, got: " + node);
    }
