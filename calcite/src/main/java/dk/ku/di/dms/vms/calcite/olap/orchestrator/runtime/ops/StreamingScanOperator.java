 ...
 * The role is explicit from the type — no mode byte or null-check needed.
 *
 * - ScanSubplan  → plain scan, rows go directly to the gateway
 * - JoinSubplan  → probe side of a broadcast hash join, combined rows go to gateway
 */
public class StreamingScanOperator implements CoordinatorOperator {
    ...
    // A7: store the two possible typed subplans. Exactly one is non-null.
    ...
    public StreamingScanOperator(VmsGatewayClient client, JoinSubPlan subplan, long snapshotId, long queryId) {
    ...
    public void open() {
        ...
        // A7: dispatch on the concrete subplan type instead of checking mode byte
        if (scanSubplan != null) {
            openScan();
        } else {
            openJoin();
    ...
    private void openJoin() {
        ...
        try {
            this.tcpIterator = client.scanWithSchema(
                    ...
                    tableName, joinSubplan.columnDescriptors(),
                    joinSubplan.predicates(), joinSubplan.routingData());
    ...
    public List<Object[]> nextBatch() {
        ...
        LOGGER.log(INFO, ">>> [StreamingScan] Blocking on tcpIterator.hasNext() for table: " + tbl + "...");
