...
/**
 * Leaf operator that streams rows from a single remote VMS. Wraps the
 * iterator returned by {@link VmsGatewayClient} in either scan mode (from a
 * {@link ScanSubPlan}) or join-receive mode (from a {@link JoinSubPlan}),
 * pulling fixed-size batches of {@code BATCH_SIZE} rows. Assigns each scan a
 * monotonic query id. Join receives reuse the id supplied by the executor.
 */
...
//        LOGGER.log(INFO, "Opening Scan -> " + host + ":" + port
...
//            LOGGER.log(INFO, "Connection Established (scan).");
        } catch (Exception e) {
...
//        LOGGER.log(INFO, "Opening Join-Receive -> " + host + ":" + port
//                + " | Table: " + tableName + " | queryId: " + queryId
//                + " | snapshotId: " + snapshotId);
...
//            LOGGER.log(INFO, "Connection Established (join).");
        } catch (Exception e) {
...
//        LOGGER.log(INFO, ">>> Blocking on tcpIterator.hasNext() for table: "
//                + tbl + "...");
        boolean hasData = tcpIterator.hasNext();
//        LOGGER.log(INFO, ">>> tcpIterator.hasNext() returned: " + hasData);
...
//            LOGGER.log(INFO, ">>> No more data. Returning null.");
            return null;
        }
...
//        LOGGER.log(INFO, ">>> Returning batch of size: " + count);
        return batch;
    }

    @Override
    public void close() {
        LOGGER.log(INFO, "Closing operator for table: " + tableName());
    }
}