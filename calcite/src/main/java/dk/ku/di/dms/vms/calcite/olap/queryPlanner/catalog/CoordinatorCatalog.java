...
/** In-memory schema catalog from the coordinator. Σchema→table metadata, table το owner-VMS
 *  placement, and snapshot id. ΓetVmsAddress resolves a table to host. Πort, falling back
 *  to the hardcoded SCHEMA_TO_ADDRESS map. */
public final class CoordinatorCatalog {
    ...
    public String getVmsAddress(String schema, String table) {
        ...
        if (resolved == null)
            resolved = SCHEMA_TO_ADDRESS.get(schema);
        if (resolved == null)
            throw new IllegalStateException(
