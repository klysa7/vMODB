...
public final class TransactionManager implements OperationalAPI, ITransactionManager {
    ...
    // -------------------------------------------------------------------------
    // B1 + B5 FIX: MVCC scan path replaces the off-heap checkpointed iterator.
    //
    // OLD: rawIndex.iterator() → IRecordIterator<IKey> → returns raw off-heap
    //      addresses (Long). Only sees records flushed to the UniqueHashBufferIndex
    //      checkpoint buffer. Any record committed after the last checkpoint() call
    //      is INVISIBLE. This broke the snapshot-isolation thesis claim.
    //
    // NEW: primaryIndex.iterator(TransactionContext(0, snapshotId, true))
    //      → Iterator<Object[]> → same MVCC path already used by getJoinIterator
    //      on the probe side. Both build and probe now read from identical
    //      snapshotId, satisfying B2 (same snapshot on both join sides).
    //
    // Return type changed from Iterator<Long> to Iterator<byte[]> so that
    // VmsQueryWorker can use the existing byteRecordIterator constructor
    ...
    public Iterator<byte[]> getScanIterator(String tableName,
        ...
        final int recordSize   = rawIndex.schema().getRecordSizeWithoutHeader();
        final int[] allOffsets = rawIndex.schema().columnOffset();

        // MVCC-aware read context — identical pattern to getJoinIterator probe side.
            ...
            public boolean hasNext() {
                ...
                while (iter.hasNext()) {
                    ...
                    if (predicates == null || predicates.isEmpty() || checkPredicates(row, predicates)) {
                        nextMatch = serializeRow(rawIndex.schema(), row, allOffsets, recordSize);
                        return true;
                    }
                }
                return false;
